    ...
    $('form').on('submit', function (e) {
       ...
       /* $("#tamplate").addClass("off")
        ...
        if (tarefasSalvas.includes(tarefa)) {
            ...
            $('input').val("")
        } else if (tarefa === "") { alert("O campo esta vazio! Por favor digite uma tarefa válida. Exemplo: 'Contratar o patrik'") }
        else {
            tarefasSalvas.push(tarefa)
            const listaDeTarefa = $(`#lista`)//seleciona a tag ul onde sera adicionada as li's
            $(`<li><p>${tarefa}</p></li>`).appendTo(listaDeTarefa)//adiciona a tag li dentro da tag ul com o valor do input (nome da tarefa) 
            $('input').val("")
        }
